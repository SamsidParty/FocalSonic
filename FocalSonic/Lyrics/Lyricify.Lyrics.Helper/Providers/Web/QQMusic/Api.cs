...
namespace Lyricify.Lyrics.Providers.Web.QQMusic
{
    public class Api : BaseApi
    {
        ...
        public async Task<MusicFcgApiResult?> Search(string keyword, SearchTypeEnum searchType)
        {
            ...
            // The search backend rate-limits per-IP and signals it with req_1 code 2001
            // (HTTP 200, empty list) — indistinguishable from "no results" without the code.
            // The token bucket refills quickly, so back off and retry a few times.
            MusicFcgApiResult? result = null;
            for (int attempt = 0; attempt < 4; attempt++)
            {
                var resp = await PostAsync("https://u.y.qq.com/cgi-bin/musicu.fcg", data);
                result = resp.ToEntity<MusicFcgApiResult>();

                if (result?.Req_1?.Code != 2001) break;
