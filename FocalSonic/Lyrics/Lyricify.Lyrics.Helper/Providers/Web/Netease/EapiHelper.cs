...
namespace Lyricify.Lyrics.Providers.Web.Netease
{
    internal class EapiHelper
    {
        public static async Task<string> PostAsync(string url, HttpClient httpClient, Dictionary<string, string> data)
        {
            ...
            // Per-request headers — the HttpClient is shared/concurrent, mutating its
            // DefaultRequestHeaders would race with other providers' requests.
            using var request = new HttpRequestMessage(HttpMethod.Post, url) { Content = new FormUrlEncodedContent(data2) };
