...
/**
 * Bundled impulse responses (public/meta/impulse), ordered from smallest to
 * largest space. Generated by Tools/generate-impulses.py, which is where the
 * character of each one is described. The ids are deliberately generic so the
 * names can be changed in en.json without invalidating saved configs, and the
 * first entry is the fallback for a config that has never picked one, so it
 * stays the most conservative.
 */
export const impulsePresets = [
    "preset1",
    "preset2",
    "preset3",
    "preset4",
...
/** Translation key holding the display name of a bundled impulse response. */
export const impulseLabelKey = (impulse: string) => `player.effects.impulses.${impulse}`;

/** The "spatial0".."spatial35" set these replaced, which is still out there in saved configs. */
const LEGACY_IMPULSE_PATTERN = /^spatial\d+$/;

/** None of the new responses match a particular old one, so they all land on the middle of the range. */
const LEGACY_IMPULSE_REPLACEMENT = "preset3";
...
export const effectiveImpulse = (impulse: string) => impulse || impulsePresets[0];

/**
 * Rewrites an id from the retired impulse set, so an old config keeps working
 * instead of falling back to FocalMK's synthetic response. Anything else,
 * including custom ids, is left alone.
 */
export const migrateImpulseId = (impulse: string) =>
    LEGACY_IMPULSE_PATTERN.test(impulse) ? LEGACY_IMPULSE_REPLACEMENT : impulse;
...
export function parseFilterData(filterData: string, speed: number): AudioEffectConfig {
    ...
    return {
        ...
        impulse: typeof rawImpulse === "string" ? migrateImpulseId(rawImpulse) : "",
...
export function isConfigDisplayable(config: AudioEffectConfig): boolean {
    ...
    if (config.impulse && !impulsePresets.includes(config.impulse)) {
        return false;
    }
...
export function validateImportedConfig(raw: unknown): ImportResult {
    ...
    // Files exported before the impulse set was replaced still name the old ids
    const impulse = migrateImpulseId(rawImpulse);

    if (impulse && !impulsePresets.includes(impulse)) {
        // A custom id only means something alongside the wav it was made from
        if (!isCustomImpulse(impulse) || rawImpulseFile === undefined) {
            return { ok: false, error: `unknown impulse response "${impulse}"` };
        }
    }
